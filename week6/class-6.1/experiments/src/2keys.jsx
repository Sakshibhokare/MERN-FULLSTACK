    ...
    return (
        <>
            ...
            {todos.map(function (todo) {
                // if we does not add key then react will get confused easily so it is very important 
                // to add a key which i can uniquely identify the element when we delete or add something 
                //it can increase the rerendering attempt 
                // key is something that will uniquely identify something for the specific things  
                return <Todo key={todo.id} title={todo.title} description={todo.description}></Todo>
                //keys will get stored to virtual dom 
                //make sure to add key when you using component multiple times
            })}
        </>
    )
}
